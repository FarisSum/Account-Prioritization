...
import { RECENT_MONTHS, RULES, SECTION_MAX } from "@/lib/scoring";
import type { GongCategory, GongSignal, ProductTelemetry } from "@/lib/types";
...
export function SignalsTabs({
  ...
  gong: GongSignal[];
  scoreHitCategories: GongCategory[];
  scoringIds: string[];
  gongPoints: number;
}) {
  const [tab, setTab] = useState<"usage" | "calls">("usage");

  return (
    <div className="mt-5">
      <div className="inline-flex gap-0.5 rounded-lg border border-zinc-200 bg-zinc-50 p-0.5 dark:border-zinc-800 dark:bg-zinc-900">
        <button type="button" onClick={() => setTab("usage")} className={tabClass(tab === "usage")}>
          Product usage
        </button>
        <button type="button" onClick={() => setTab("calls")} className={tabClass(tab === "calls")}>
          Call signals
          <span className="ml-1.5 tabular-nums text-xs opacity-60">{gong.length}</span>
        </button>
      </div>
...
function UsagePanel({ telemetry }: { telemetry: ProductTelemetry | null }) {
  return (
      ...
      {!telemetry ? (
        ...
        <div className="mt-4 space-y-5">
          ...
          <MetricGroup title="Recurring & vaulting">
            ...
            <MetricCard
              label="Tokens stored"
              value={telemetry.tokens_stored === null ? "—" : formatCompact(telemetry.tokens_stored)}
            />
            <MetricCard label="Active dashboard users" value={telemetry.active_users ?? "—"} />
            <MetricCard label="Report exports / mo" value={telemetry.reporting_exports_monthly ?? "—"} />
          </MetricGroup>
...
function CallsPanel({
  ...
  const hitSet = new Set(scoreHitCategories);
  const scoringIdSet = new Set(scoringIds);
    ...
    <section className="mt-4">
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        <div className="flex gap-3 text-xs text-zinc-500 dark:text-zinc-400">
          <span className="text-emerald-600 dark:text-emerald-400">{counts.Positive} positive</span>
          <span>{counts.Neutral} neutral</span>
          <span className="text-red-600 dark:text-red-400">{counts.Negative} negative</span>
        </div>
      </div>
      <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
        ...
        detected in the last {RECENT_MONTHS} months adds {RULES.gong.pointsPerCategory} points to the
        score, capped at one hit per category —{" "}
        <span className="font-medium text-zinc-700 dark:text-zinc-200">
          {hitSet.size}/{CATEGORY_ORDER.length} categories, +{gongPoints}
        </span>{" "}
        here.
      </p>
              ...
              <div key={category}>
                ...
                <ul className="space-y-2">
                  {byCategory.get(category)!.map((g) => {
                    const isScoring = scoringIdSet.has(g.transcript_id);
                    return (
                      <li
                        key={g.transcript_id}
